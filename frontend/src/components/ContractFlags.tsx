...
const ContractFlags: React.FC<ContractFlagsProps> = ({
  ...
  const flagConfig = {
    disqualifying: {
      ...
      label: 'Disqualifying',
      flags: [
        { value: DisqualifyingFlag.NOT_FIT_TECH_STACK, label: 'Not Fit - Tech Stack' },
        { value: DisqualifyingFlag.NOT_FIT_COMPLIANCE, label: 'Not Fit - Compliance' },
        { value: DisqualifyingFlag.NOT_FIT_TIMELINE, label: 'Not Fit - Timeline' },
        { value: DisqualifyingFlag.NOT_FIT_TOO_COMPLEX, label: 'Not Fit - Too Complex' },
        { value: DisqualifyingFlag.REQUIRES_PARTNER, label: 'Requires Partner' },
        { value: DisqualifyingFlag.NO_BUDGET_CEILING, label: 'No Budget Ceiling' },
      ]
    },
    strategic: {
    ...
    postAward: {
      ...
      flags: [
        ...
        { value: PostAwardFlag.LOST_DISQUALIFIED, label: 'Lost - Disqualified' },
  ...
  const getFlagLabel = (flag: ContractFlag): string => {
    ...
    const flagInfo = flagConfig[category].flags.find((f: any) => f.value === flag);
  ...
  const handleAddFlag = async (flag: ContractFlag) => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/api/contracts/${contractId}/flags`, {
        method: 'PUT',
        headers: {
  ...
  const renderFlagsByCategory = () => {
    ...
    return Object.entries(flagConfig).map(([category, config]) => {
      const categoryFlags = categorizedFlags[category as keyof typeof flagConfig] || [];
