...
const ContractStatusSelector: React.FC<ContractStatusSelectorProps> = ({
  ...
  const statusConfig = {
    ...
    [ContractStatus.AWARDED]: {
      ...
      buttonColor: "hover:bg-emerald-50 dark:hover:bg-emerald-800",
    },
    [ContractStatus.LOST]: {
      label: "Lost",
      color:
              ...
              <div className="p-1">
                {Object.entries(statusConfig).map(([statusValue, config]) => (
                  <button
                    key={statusValue}
                    onClick={() =>
                      handleStatusChange(statusValue as ContractStatus)
                    }
